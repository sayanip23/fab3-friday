...
with tabs[0]:
    ...
    _prose, _falls = st.columns([1, 1], gap="large")
    ...
    # ═══════════════════════════════════════════════════════════ waterfall
    # The paragraph, drawn. Bars step down from the prior value through each
    # lever and must land exactly on the current value — which is the zero
    # residual claim made visible rather than asserted in a table.
    #
    # Only drawn when the decomposition genuinely reconciles to THIS KPI's
    # movement. price_volume_mix always splits revenue, so on a ratio or a unit
    # count KPI the steps would not sum to the movement and the picture would be
    # a lie. Deleting this block restores the previous full width narrative.
    with _falls:
        _recon = abs(pvm.total_movement - movement.delta) <= max(
            1e-6 * abs(movement.delta), 1e-6)

        if not _recon:
            st.caption(
                f"No waterfall for {movement.label}: price, volume and mix "
                f"decomposes revenue, and this KPI is measured in "
                f"{movement.unit}. Drawing the steps against a total they do not "
                f"sum to would misrepresent them, so it is withheld.")
        else:
            _run = movement.prior
            _levels = [_run]
            _steps = []
            for _e in sorted(pvm.effects, key=lambda x: abs(x.value), reverse=True):
                _lo, _hi = (_run + _e.value, _run) if _e.value < 0 else (_run, _run + _e.value)
                _run += _e.value
                _levels.append(_run)
                _steps.append({"step": _e.name, "lo": _lo, "hi": _hi,
                               "amount": _e.value,
                               "kind": "down" if _e.value < 0 else "up"})

            # The prior and current bars are drawn from a floor just under the
            # lowest level, not from zero. Anchored at zero the axis spans the
            # whole 8m and the three levers — which are the entire point — become
            # slivers a few pixels tall beside two full height columns.
            _span = max(_levels) - min(_levels) or abs(max(_levels)) or 1.0
            _floor = min(_levels) - 0.35 * _span
            _ceil = max(_levels) + 0.12 * _span

            _wf = ([{"step": "Prior", "lo": _floor, "hi": movement.prior,
                     "amount": movement.prior, "kind": "total"}]
                   + _steps
                   + [{"step": "Current", "lo": _floor, "hi": _run,
                       "amount": _run, "kind": "total"}])
            _wfd = pd.DataFrame(_wf)
            _wfd["top"] = _wfd[["lo", "hi"]].max(axis=1)

            _order = list(_wfd.step)
            _base = alt.Chart(_wfd).encode(
                x=alt.X("step:N", sort=_order, title=None,
                        axis=alt.Axis(labelAngle=0, labelFontSize=11)))
            _bars = _base.mark_bar(size=34, cornerRadius=2).encode(
                # zero=False so the steps are readable: against a zero baseline a
                # 19% fall on a 8.1m base is three near identical full height bars
                y=alt.Y("lo:Q", title=f"{movement.label} ({PVM_UNIT})",
                        scale=alt.Scale(domain=[_floor, _ceil], nice=False,
                                        clamp=True)),
                y2="hi:Q",
                color=alt.Color("kind:N", legend=None, scale=alt.Scale(
                    domain=["total", "down", "up"],
                    range=[theme.PURPLE_MID, DOWN, UP])),
                tooltip=[alt.Tooltip("step:N", title="step"),
                         alt.Tooltip("amount:Q", format=",.0f", title="amount")])
            _text = _base.mark_text(dy=-7, fontSize=11, fontWeight="bold",
                                    color=theme.INK).encode(
                y=alt.Y("top:Q", scale=alt.Scale(domain=[_floor, _ceil],
                                                 nice=False, clamp=True)),
                text=alt.Text("amount:Q", format=",.0f"))

            st.altair_chart((_bars + _text).properties(height=232),
                            width="stretch")
            st.caption(
                f"Prior, each lever, then current. The steps land on "
                f"{fmt(movement.current)} exactly — residual "
                f"{abs(pvm.residual):.6f}. An identity, not an estimate.")
    # ═══════════════════════════════════════════════════════ end waterfall
